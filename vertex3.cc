...
int tcool=300000; /* start time for cooling (end of spring extension) */
int TT   =440000; /* total time period for simulation */
...
double upg0=4.0; /* water pressure during spring extension */
double upg_cool=0.1;  /* water pressure during cooling */
double shift_upg=10000; /* steepness of switching from upg0 to upg_cool */
double upg=upg0;
...
double amp_tw_side=0.06; /* degree of twist for side horns */
...
double morpha_deko(double x,double y){
  ...
  value+=0.4*expfa(x,y,0.3,0.2,dx11,dx11,dy11,dy11,sx0,0.07,sy0,0.07,20);
  value+=0.7*expfa(x,y,0.3,0.2,3*dx11,3*dx11,dy11*0.9,dy11*0.9,sx0,0.07,sy0,0.07,20);
  ...
  value+=0.4*expfa(x,y,0.3,0.2,-1*dx11,-1*dx11,dy11,dy11,sx0,0.07,sy0,0.07,20);
  value+=0.7*expfa(x,y,0.3,0.2,-3*dx11,-3*dx11,dy11*0.9,dy11*0.9,sx0,0.07,sy0,0.07,20);
  value+=0.7*expfa(x,y,0.3,0.2,-6*dx11,-6*dx11,dy11*0.8,dy11*0.8,sx0,0.07,sy0,0.07,20);
  value+=0.4*expfa(x,y,0.3,0.2,-6*dx11,-8*dx11,dy11*0.7,dy11*0.7,sx0,0.07,sy0,0.07,20);
													  return value;
...
double morpha_sideL(double x,double y){
    rotate(x,y);  
      ...
      double value=0.0;
      value+=1*expfa(x,y,0.3,0.2,-6*dx11,-6*dx11,dy11*0.5,dy11*0.5,0.3,0.07,0.3,0.07,20);
      value+=1*expfa(x,y,0.3,0.2,-6*dx11,-6*dx11,dy11*0.8,dy11*0.8,0.3,0.07,0.3,0.07,20);
      return value;
 }
...
int main(int argc, char *argv[]){
  ...
  /* calculation of spring growth correspongind to sheet growth */
  ...
  develop(nabla2,morpha_head,d_gradx_cen,2*d_grady_cen,amp_tw_cen);//beta5

  /* calculation of growth metric correspongind to sheet growth */
  for(int i=0;i<nn*nn;i++){
    met[i][0]=0.0;
    met[i][1]=0.0;
    metric[i][0]=0.0;
  ...
  calc_met(morpha_head,d_gradx_cen,2*d_grady_cen,amp_tw_cen);//beta5
